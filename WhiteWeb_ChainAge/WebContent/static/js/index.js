...
function xx(data) {
    ...
    $.each(jsonObj, function (i, n) {
        ...
        card.find(' .logo img').attr('src', 'img/coin/128/' + n.specificRate.baseId + '.png');
        card.find(' .chart').draw(xxxxxxx(n.series), '#3B3B3B', '#3B3B3B');
    });
}
...
function rankShowFunc(data) {
    ...
    for (var i = 0; i < 5; i++) {
        ...
        rankLHtml += '<div class="ranking-list">'
            + '<div class="ranking-num ' + numClass + '">' + (i + 1) + '</div>'
            + '<div class="ranking-icon">'
            + '  <img src="img/coin/128/' + data[i].exchangeId + '.png" alt="" width="20px" height="20px">'
            + '</div>'
    ...
    for (var i = 5; i < 10; i++) {
        rankRHtml += '<div class="ranking-list">'
